...
Ltac reshape_expr e tac :=
  ...
  let rec go K vs e :=
    ...
    | App ?e (of_val ?v)                 => add_item (AppLCtx v) vs K e
    ...
    | Pair ?e (of_val ?v)                => add_item (PairLCtx v) vs K e
    ...
    | Fold ?e                         => add_item (FoldCtx) vs K e
    | Unfold ?e                         => add_item (UnfoldCtx) vs K e
...
From fae_gtlc_mu.backtranslation Require Export cast_help.extract cast_help.embed.
...
for each possible decomposition until [tac] succeeds. *)
 ...
 Lemma wp_extract_TUnit_embed_TProd v1 v2 Φ :
   (True -∗ WP (extract TUnit Ground_TUnit (embedV_Ground_TProd (stlc_mu.lang.PairV v1 v2))) {{ Φ }})%I.
 Proof.
   iIntros.
   wp_head.
   asimpl.
 ...
 Lemma wp_extract_TUnit_embed_TUnit Φ : (WP Un -∗ (WP Ω {{ Φ }}))%I.
