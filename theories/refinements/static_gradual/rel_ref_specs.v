...
Section relation_for_specification_lemma.
  ...
  (* Theorem embedding_relates (Γ : list stlc_mu.typing.type) pΓ (e : stlc_mu.lang.expr) (τ : stlc_mu.typing.type) pτ : *)
  ...
  Theorem back_relates (Γ : list cast_calculus.types.type) pΓ (e : cast_calculus.lang.expr) (τ : cast_calculus.types.type) pτ :
    Γ & pΓ ⊢ₜ e : τ & pτ → Γ ⊨ <<e>> ≤log≤ e : τ.
  Proof.
    induction 1; simpl.
    ...
    - eapply bin_log_related_injr; eauto.
    - eapply bin_log_related_case; try done.
      cut (∀ f : var → stlc_mu.lang.expr, (<< e1 >>).[upn (length (map backtranslate_type (τ1 :: Γ))) f] = << e1 >>).
      { rewrite map_length. by simpl. }
      eapply stlc_mu.typing.typed_n_closed. apply well_typedness_expr. apply H0.
      cut (∀ f : var → stlc_mu.lang.expr, (<< e2 >>).[upn (length (map backtranslate_type (τ2 :: Γ))) f] = << e2 >>).
      { rewrite map_length. by simpl. }
      eapply stlc_mu.typing.typed_n_closed. apply well_typedness_expr. apply H1.
      rewrite -(cons_length τ1). by eapply cast_calculus.typing.typed_n_closed.
      rewrite -(cons_length τ2). by eapply cast_calculus.typing.typed_n_closed.
    - eapply bin_log_related_lam; eauto.
      cut (∀ f : var → stlc_mu.lang.expr, (<< e >>).[upn (length (map backtranslate_type (τ1 :: Γ))) f] = << e >>).
      { rewrite map_length. by simpl. }
      eapply stlc_mu.typing.typed_n_closed. apply well_typedness_expr. apply H.
      rewrite -(cons_length τ1). by eapply cast_calculus.typing.typed_n_closed.
    - eapply bin_log_related_app; eauto.
    - eapply bin_log_related_fold; eauto.
    - eapply bin_log_related_unfold; eauto.
    - destruct (decide (cons_stand_open τi τf));
